...
if ($product['category_id']) {
    ...
    $current_cat_stmt->execute([$product['category_id']]);
    ...
    <script>
        ...
        async function updateModelGroups(brandId, selectedGroupId = null) {
            ...
            try {
                ...
                if (selectedGroupId) updateSpecificModels(selectedGroupId, <?= $product['category_id'] ?>);
            } catch (error) {
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-10">
            <header class="mb-10 flex justify-between items-center">
                <h1 class="text-3xl font-bold text-gray-800 tracking-tight">Edit <span class="text-primary-600">Product</span></h1>
            ...
            <div class="bg-white rounded-[3rem] shadow-sm border border-gray-200 p-12 max-w-5xl relative overflow-hidden">
                ...
                <form action="admin-product-edit.php?id=<?= $id ?>" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-12 relative">
                    <div class="space-y-8">
                        <div>
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Product Name</label>
                            <input type="text" name="name" value="<?= htmlspecialchars($product['name']) ?>" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" step="0.01" name="price" value="<?= $product['price'] ?>" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Stock Count</label>
                                <input type="number" name="stock" value="<?= $product['stock_quantity'] ?>" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all">
                            </div>
                        </div>

                        <div class="flex items-center gap-3 bg-primary-50/50 p-4 rounded-2xl border border-primary-100">
                            <input type="checkbox" name="is_featured" id="is_featured" <?= $product['is_featured'] ? 'checked' : '' ?> class="w-5 h-5 text-primary-600 border-gray-300 rounded-lg focus:ring-primary-500">
                    ...
                    <div class="space-y-8">
                        <div>
                            ...
                            <input type="text" name="image_url" value="<?= htmlspecialchars($product['main_image']) ?>" placeholder="assets/images/products/..." class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all">
                        </div>

                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">SKU</label>
                                <input type="text" name="sku" value="<?= htmlspecialchars($product['sku']) ?>" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 font-mono text-sm transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Part #</label>
                                <input type="text" name="part_number" value="<?= htmlspecialchars($product['part_number']) ?>" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 font-mono text-sm transition-all">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Description</label>
                            <textarea name="description" rows="3" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all"><?= htmlspecialchars($product['description']) ?></textarea>
                        </div>
                        ...
                        <div class="space-y-6 bg-gray-50/50 p-8 rounded-[2rem] border border-gray-100">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="quality_tier" value="<?= htmlspecialchars($product['quality_tier']) ?>" class="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-sm">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Warranty</label>
                                    <input type="text" name="warranty" value="<?= htmlspecialchars($product['warranty']) ?>" class="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-sm">
                                </div>
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Bulk Pricing (JSON)</label>
                                <textarea name="bulk_pricing" rows="2" class="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 font-mono text-[10px]"><?= htmlspecialchars($product['bulk_pricing']) ?></textarea>
                            </div>
                        </div>
                    </div>
