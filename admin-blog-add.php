...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO blog_posts (title, slug, excerpt, content, author, featured_image, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if ($stmt->execute([$title, $slug, $excerpt, $content, $author, $featured_image, $status])) {
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-10">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-200 p-10 max-w-4xl">
                <form action="admin-blog-add.php" method="POST" class="space-y-6">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Title</label>
                        <input type="text" name="title" value="<?= $_POST['title'] ?? '' ?>" required class="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-primary-500">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Author</label>
                            <input type="text" name="author" value="<?= $_POST['author'] ?? 'VisionPro Team' ?>" class="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-primary-500">
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="featured_image" value="<?= $_POST['featured_image'] ?? '' ?>" placeholder="https://example.com/image.jpg" class="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-primary-500">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Excerpt (Short Description)</label>
                        <textarea name="excerpt" rows="3" class="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-primary-500"><?= $_POST['excerpt'] ?? '' ?></textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Content</label>
                        <textarea name="content" id="content" rows="10" class="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-primary-500 font-mono text-sm" placeholder="Write your blog content here... You can use HTML tags like &lt;p&gt;, &lt;br&gt;, &lt;b&gt;, &lt;i&gt;, &lt;ul&gt;, &lt;li&gt;, etc."><?= $_POST['content'] ?? '' ?></textarea>
