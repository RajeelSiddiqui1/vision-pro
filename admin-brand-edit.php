...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        $stmt = $pdo->prepare("UPDATE brands SET name = ?, slug = ?, logo = ?, description = ?, is_active = ? WHERE id = ?");
        if ($stmt->execute([$name, $slug, $logo_url, $description, $is_active, $id])) {
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-10">
            <header class="mb-10 flex justify-between items-center">
                <h1 class="text-3xl font-bold text-gray-800 tracking-tight">Modify <span class="text-primary-600">Brand Entity</span></h1>
            ...
            <div class="bg-white rounded-[3rem] shadow-sm border border-gray-200 p-12 max-w-2xl relative overflow-hidden">
                ...
                <form action="admin-brand-edit.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data" class="space-y-8 relative">
                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Brand Name</label>
                        <input type="text" name="name" value="<?= htmlspecialchars($brand['name']) ?>" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all">
                    </div>

                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Brand Description</label>
                        <textarea name="description" rows="3" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl outline-none focus:ring-2 focus:ring-primary-500 font-bold text-gray-700 transition-all"><?= htmlspecialchars($brand['description']) ?></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <?php if ($brand['logo']): ?>
                            <div class="mb-6 p-4 bg-gray-50 rounded-2xl border border-gray-100 w-fit">
                                <img src="<?= $brand['logo'] ?>" class="h-16 w-auto object-contain">
                            </div>
                        <?php endif; ?>
                        <div class="relative group">
                            ...
                            <div class="w-full px-6 py-8 border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center gap-3 group-hover:bg-gray-50 group-hover:border-primary-300 transition-all">
                                <i class="ri-refresh-line text-3xl text-gray-300 group-hover:text-primary-500 transition-transform group-hover:rotate-180 duration-500"></i>
                                <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Click to replace current logo</span>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-3 bg-gray-50 p-4 rounded-2xl border border-gray-100 w-fit">
                        <input type="checkbox" name="is_active" id="is_active" <?= $brand['is_active'] ? 'checked' : '' ?> class="w-5 h-5 text-primary-600 border-gray-300 rounded-lg focus:ring-primary-500">
                        <label for="is_active" class="text-xs font-black text-gray-500 uppercase tracking-widest cursor-pointer">Entity Active</label>
                    </div>

                    <div class="pt-4 flex items-center gap-6">
                        <div class="flex-1 h-px bg-gray-100"></div>
                        <button type="submit" class="px-12 py-5 bg-gray-900 text-white font-black rounded-2xl hover:bg-primary-600 shadow-xl shadow-gray-200 hover:shadow-primary-200 transition-all uppercase text-xs tracking-[0.3em]">
                            Update Identity
                        </button>
                        <div class="flex-1 h-px bg-gray-100"></div>
                    </div>
