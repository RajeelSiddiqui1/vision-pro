...
// Get service data
$stmt = $pdo->prepare("SELECT * FROM repair_services WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($slug) || $price <= 0) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM repair_services WHERE slug = ? AND id != ?");
...
<head>
    ...
    <title>Edit Repair Service - VisionPro Admin</title>
        ...
        <main class="flex-1 p-10">
            <header class="flex justify-between items-center mb-10">
                <div>
                    ...
                    <h1 class="text-3xl font-bold text-gray-800">Edit Repair Service</h1>
                </div>
            </header>
                ...
                <form method="POST" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" required value="<?= htmlspecialchars($service['name']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="e.g., Screen Repair">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Slug *</label>
                            <input type="text" name="slug" required value="<?= htmlspecialchars($service['slug']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="e.g., screen-repair">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="Describe the service..."><?= htmlspecialchars($service['description'] ?? '') ?></textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Price ($) *</label>
                            <input type="number" name="price" step="0.01" min="0" required value="<?= $service['price'] ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="99.00">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Duration (minutes)</label>
                            <input type="number" name="duration_minutes" min="1" value="<?= $service['duration_minutes'] ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="60">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Icon</label>
                            <input type="text" name="icon" value="<?= htmlspecialchars($service['icon']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="📱">
                            <p class="text-xs text-gray-500 mt-1">Emoji icon to display</p>
                        </div>
                    </div>

                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" <?= $service['is_active'] ? 'checked' : '' ?> class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500">
                        <label for="is_active" class="ml-2 text-sm font-bold text-gray-700">Service is Active</label>
                    </div>

                    <div class="flex gap-4">
                        <button type="submit" class="bg-primary-600 text-white px-8 py-3 rounded-lg font-bold hover:bg-primary-700 transition-colors">Update Service</button>
